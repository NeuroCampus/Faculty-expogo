...
export const ClassSelector: React.FC<Props> = ({ value, onChange }) => {
  const { data, isLoading } = useQuery({ queryKey:['assignments'], queryFn: async () => { const r = await getAssignments(); if(!r.success) throw new Error(r.error); return r.data?.data || []; }});
  ...
  return (
    <View style={{ marginBottom:12 }}>
      <Text style={{ fontFamily:'System', marginBottom:4 }}>Branch</Text>
      ...
      {semesters.length>0 && <>
        <Text style={{ fontFamily:'System', marginBottom:4, marginTop:8 }}>Semester</Text>
      ...
      {sections.length>0 && <>
        <Text style={{ fontFamily:'System', marginBottom:4, marginTop:8 }}>Section</Text>
      ...
      {subjects.length>0 && <>
        <Text style={{ fontFamily:'System', marginBottom:4, marginTop:8 }}>Subject</Text>
