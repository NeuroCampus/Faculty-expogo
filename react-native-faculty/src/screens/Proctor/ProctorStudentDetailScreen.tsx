...
export default function ProctorStudentDetailScreen({ route }: any){
  ...
  return (
    <ScrollView style={{ flex:1, padding:16 }}>
      <Text style={{ fontSize:22, fontFamily:'Inter-SemiBold', marginBottom:8 }}>{student.name}</Text>
      <Text style={{ marginBottom:8 }}>{student.usn}</Text>
      <Text style={{ fontFamily:'Inter-SemiBold', marginTop:12 }}>Attendance</Text>
      <Text>{student.attendance ?? '--'}%</Text>
      <Text style={{ fontFamily:'Inter-SemiBold', marginTop:12 }}>Marks</Text>
      {(student.marks||[]).map((m:any, idx:number)=>(<Text key={idx} style={{ fontSize:12 }}>{m.subject} T{m.test_number}: {m.mark}/{m.max_mark}</Text>))}
      <Text style={{ fontFamily:'Inter-SemiBold', marginTop:12 }}>Latest Leave</Text>
      ...
      <Text style={{ fontFamily:'Inter-SemiBold', marginTop:12 }}>Certificates</Text>
