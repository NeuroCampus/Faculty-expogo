...
export default function AssignmentsScreen() {
  ...
  async function load() {
    ...
    if (r.ok && r.data?.data) setTasks(r.data.data); else setTasks([]);
    setLoading(false);
  }
  useEffect(()=> { load(); }, []);
  ...
  async function openTask(t: AssignmentTask) {
    ...
    if (subs.ok && subs.data?.data) setSubmissions(subs.data.data); else setSubmissions([]);
  }
  ...
  return (
    <View style={{ flex:1, padding:16 }}>
      {!activeTask && (
        <>
          <Text style={{ fontSize:22, fontFamily:'Inter-SemiBold', marginBottom:12 }}>Assignments</Text>
          ...
          <FlatList data={tasks} keyExtractor={i=> String(i.id)} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=> { setRefreshing(true); load().finally(()=> setRefreshing(false)); }} />} renderItem={({ item }) => (
            <TouchableOpacity onPress={()=> openTask(item)} style={{ padding:14, backgroundColor:'#f1f5f9', borderRadius:12, marginBottom:10 }}>
              <Text style={{ fontFamily:'Inter-SemiBold', fontSize:16 }}>{item.title}</Text>
      ...
      {activeTask && (
        <View style={{ flex:1 }}>
          ...
          <Text style={{ fontSize:20, fontFamily:'Inter-SemiBold' }}>{activeTask.title}</Text>
          ...
          <Text style={{ marginTop:16, fontFamily:'Inter-SemiBold' }}>Submissions</Text>
