...
export default function ExamsScreen() {
  ...
  return (
    <View style={{ flex:1, padding:16 }}>
      {!activeExam && (
        <>
          <Text style={{ fontSize:22, fontFamily:'System', marginBottom:12 }}>Exams</Text>
          ...
          <FlatList data={exams} keyExtractor={i=> String(i.id)} renderItem={({ item }) => (
            <TouchableOpacity onPress={()=> openExam(item)} style={{ padding:14, backgroundColor:'#f1f5f9', borderRadius:12, marginBottom:10 }}>
              <Text style={{ fontFamily:'System', fontSize:16 }}>{item.name}</Text>
      ...
      {activeExam && (
        <View style={{ flex:1 }}>
          ...
          <Text style={{ fontSize:20, fontFamily:'System' }}>{activeExam.name}</Text>
          <Text style={{ color:'#475569', marginTop:4 }}>{activeExam.date || 'No date.'}</Text>
          <Text style={{ marginTop:16, fontFamily:'System' }}>Results</Text>
