  ...
  try {
    ...
    if (response.status === 401) {
      ...
      if (refreshResult.success && refreshResult.access) {
        await AsyncStorage.setItem("access_token", refreshResult.access);
        if (refreshResult.refresh) {
          await AsyncStorage.setItem("refresh_token", refreshResult.refresh);
        }
        options.headers = {
          ...options.headers,
...
export const loginUser = async ({ username, password }: LoginRequest): Promise<LoginResponse> => {
  ...
  try {
    ...
    if (response.ok && result.success) {
      ...
      await AsyncStorage.setItem("user", JSON.stringify(result.profile || {}));
      startTokenRefresh();
    }
    return result;
  } catch (error: any) {
